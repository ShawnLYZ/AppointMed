 ...
 * relaxations are needed and exactly 3 ollama decisions are consumed
 * (completeIntake, the triage verdict, then prefs).
 */
...
test('selecting a slot books via the adapter with the right hospital key and creates a pending request', async () => {
  ...
  ctx.ollama.enqueue({ summary: 'Male patient, exertional chest tightness 2 weeks, severity 6, hypertensive on amlodipine. Cardiology review advised within a week.', priority: 'medium' });
